...
# Parse the command-line flags using getopts
while getopts "i:s:d:t:w:o:r:g:p:u:" opt; do
  case $opt in
    i) input_table="$OPTARG" ;;  # -i for input_table
    s) script_path="$OPTARG" ;;  # -s for script_path
    d) dat_type="$OPTARG" ;;  # -d for dat_type
    t) tract_path="$OPTARG" ;;  # -t for tract_path
    w) window_slices="$OPTARG" ;;  # -w for window_slices
    o) output_path="$OPTARG" ;;  # -o for output_path
    r) display_brain="$OPTARG" ;;  # -r for display_brain
    g) group_to_process="$OPTARG" ;;  # -g for group_to_process
    p) template="$OPTARG" ;;  # -p for template
    u) overwrite="$OPTARG" ;;  # -u for overwrite
    \?)  # Invalid option
      echo "Usage: $0 -i <input_table> -s <scripts_dir> -d <dat_type> -t <tract_path> -w <window_slices> -o <output_path> -r <display_brain> -g <group_to_process> -p <template_space> -u <overwrite>"
      exit 1
      ;;
  esac
done
...
if [[ -z "$input_table" || -z "$script_path" || -z "$dat_type" || -z "$tract_path" || -z "$window_slices" || -z "$output_path" || -z "$display_brain" || -z "$group_to_process" || -z "$template" || -z "$overwrite" ]]; then
  echo "Error: Missing required options."
  echo "Usage: $0 -i <input_table> -s <scripts_dir> -d <dat_type> -t <tract_path> -w <window_slices> -o <output_path> -r <display_brain> -g <group_to_process> -p <template_space> -u <overwrite>"
  exit 1
fi

# Your script continues here using the variables set by getopts...
echo "Input Table: $input_table"
echo "Script Path: $script_path"
echo "Data Type: $dat_type"
echo "Tract Path: $tract_path"
echo "Window Slices: $window_slices"
echo "Output Path: $output_path"
echo "Display Brain: $display_brain"
echo "Group to Process: $group_to_process"
