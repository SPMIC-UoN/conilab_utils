...
output="${StudyFolder}/blueprint_averagingWBC_atlas"
if [ -d $output ]; then
...
ave_split () {
	...
	do
    	bpFile="${StudyFolder}/${subID}/MNINonLinear/Results/blueprint_atlas/bpTracts.dtseries.nii"
    	if [ -f $bpFile ]; then
...
/usr/local/workbench/bin_rh_linux64/wb_command -cifti-average ${output}/ave_blueprint_atlas.dtseries.nii -cifti ${output}/ave_1.dtseries.nii -cifti ${output}/ave_2.dtseries.nii -cifti ${output}/ave_3.dtseries.nii -cifti ${output}/ave_4.dtseries.nii -cifti ${output}/ave_5.dtseries.nii
