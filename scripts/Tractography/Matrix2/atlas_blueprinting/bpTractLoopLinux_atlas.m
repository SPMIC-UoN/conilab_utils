...
for i=1:size(tracts,2)
    ...
    t_in=([PtxFolder '/' tracts{i}]);
    tempFolder=([PtxFolder '/' tracts{i} '_resampled_' num2str(subID)]);
...
bpPath=fullfile(DiffStudyFolder, num2str(subID),'MNINonLinear/Results/blueprint_atlas',...
...
if side=='RH'
    ...
    try
        ...
        savebpCiiLinux_atlas(bpPath, subID, StrucStudyFolder, dim, ds, dt, tracts);
    catch
...
%% RMSE between individual and atlas blueprints
